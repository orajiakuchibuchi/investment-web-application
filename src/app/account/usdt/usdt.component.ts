...
export class UsdtComponent implements OnInit {
  ...
  onSubmit(){
    if(this.validate().address.valid){
      ...
      this.BaseApiService.updateUsdtAddress(formData).subscribe(data=>{
        if(data['status'] == '200'){
  ...
  delete(){
    ...
    this.BaseApiService.deleteBtcAddress(formData).subscribe(data=>{
      if(data['status'] == '200'){
        this.address = [];
        this.isAdding = false;
      }else{
